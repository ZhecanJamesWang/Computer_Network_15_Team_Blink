...
#routing table
dict_routing_table = {"A":"192.168.128.103", "B":"192.168.128.104", "C":"192.168.128.111", "D":"192.168.128.102"}
...
def search_routing_table(message):
    ...
    dst_lan = dst_ip[0]
    dst_router_ip = dict_routing_table[dst_lan]
...
def thread_socket():
    ...
    with socket(AF_INET, SOCK_DGRAM) as sock:
        ...
        sock.bind((IP,port))            
        sock.settimeout(2.0) 
        print ("Router started on IP Address {}, port {}".format(IP,port))
        while True:
            try:
                bytearray_msg, source_address = sock.recvfrom(1024)
                source_IP, source_port = source_address
                print ("Packet received from ip address {}, port {}:".format(source_IP,source_port))
